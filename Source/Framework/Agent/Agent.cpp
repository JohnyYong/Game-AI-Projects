...
Agent::Agent(const char *type, size_t id) : position(0.0f, 0.0f, 0.0f),
    scaling(3.0f, 3.0f, 3.0f), eulerAngles(0.0f, 0.0f, 0.0f), isDirty(true), color(0.7f, 0.7f, 0.7f), type(type), id(id), movementSpeed(2000.0f / 2.3f), health(100.f),
    maxHealth(100.f)
{}
...
float Agent::get_maxHealth() const
{
    return maxHealth;
}

//Added by Student (Johny Yong Jun Siang)
float Agent::get_health() const
{
    return health;
}
...
//Added by Student (Johny Yong Jun Siang)
void Agent::set_health(float initialHealth)
{
    health = initialHealth;
}

void Agent::set_maxHealth(float newMaxHealth)
{
